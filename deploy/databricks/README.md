...
This copy is adjusted for Databricks Free Edition defaults: use the
`workspace` catalog, default to `MEDIUM` app compute, and keep Apps
telemetry export disabled unless the workspace has a custom storage
location.
...
3. The repo's local venv with the `databricks` extra:
   `uv sync --extra databricks` or run commands through
   `uv run --extra databricks` (use `uv`, not global pip).
...
```sql
CREATE SCHEMA IF NOT EXISTS workspace.omnigent;
CREATE VOLUME IF NOT EXISTS workspace.omnigent.artifacts;
```
...
uv run --extra databricks python deploy/databricks/deploy.py --skip-build --allow-dirty ...

# API-only deploy (drops the SPA from the main wheel).
uv run --extra databricks python deploy/databricks/deploy.py --skip-web-ui ...
```
